...
check_debian_release()
{
    ...
    if is_ubuntu && [ "$VERSION_ID" != 16.04 ] && [ "$VERSION_ID" != 18.04 ] && [ "$VERSION_ID" != 20.04 ]
    then
        exit_with_error "You are running unsupported version of debian based OS. $PRODUCT supports only Ubuntu 16.04, 18.04 and 20.04"
    fi
...
check_centos_compatible()
{
    ...
    [ "$1" -lt 9 ] 2>/dev/null && [ "$1" -gt 5 ] || \
        exit_with_error "Only CentOS/CloudLinux 6, 7, and 8 are supported at the moment, but got os_version='$1'"
}
...
check_centos_release ()
{
    ...
    check_centos_compatible "$OS_VERSION"
}
        ...
        if [[ "$package" = "$imunify360" ]]; then
            if [ -f '/etc/cpanel/ea4/is_ea4' ] ; then
                ...
                then
                    ...
                    log install_${ostype}_pkgs ea-apache24-mod_security2
                fi
            fi
        fi
